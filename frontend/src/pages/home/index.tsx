...
export default function Home() {
  ...
  const handleSearchModeChange = (mode: 'both' | 'concepts' | 'author') => {
    setSearchMode(mode);
    // Limpiar inputs cuando cambia el modo
    setQuery("");
    setResults([]);
    setSelected([]);
    setAuthorVal("");
      ...
      <div className="flex items-start justify-center p-4 pt-8">
        <div className="w-1/2">
          	<div className="bg-white rounded-xl border-2 border-gray-300 p-6 mb-6">
				...
				<p>Genera recomendaciones de investigadores con los que colaborar a partir de tus temas de interés en inteligencia artificial, o ingresa tu nombre (o el de un investigador conocido) para recibir sugerencias basadas en la red de colaboración científica en Latinoamérica.
				</p>
				</div>
				<label className="block font-semibold my-3 text-gray-500">Buscar por:</label>
				...
				>
					...
					<div>Ambos</div>
					<div className="text-gray-500 text-sm">Conceptos y autor</div>
				</button>
				...
				>
					...
					<div>Conceptos</div>
					<div className="text-gray-500 text-sm">Solo temas de interés</div>
				</button>
				...
				>
					...
					<div className="text-gray-500 text-sm">Solo nombre de autor</div>
				</button>
				</div>
			</div>
			<form onSubmit={searchRecommended} className="bg-white rounded-xl border-2 border-gray-300 p-8">
				{(searchMode === 'both' || searchMode === 'concepts') && (
					<div className="mb-6">
						<label htmlFor="concepts" className="block mb-2 text-gray-700">
						...
						<div className="relative">
							...
							<input
								...
								placeholder="Escribe un concepto y haz click en él para seleccionarlo..."
						...
						<div className="mt-3">
							...
							Conceptos seleccionados ({selected.length}):
							<div>
								<button className="bg-indigo-400 hover:bg-indigo-600 px-4 py-1 text-white rounded-lg cursor-pointer disabled:bg-gray-300" onClick={cleanConcepts}>Limpiar</button>
							</div>
							</div>
							...
							{selected.map((concept) => (
								<span
								key={concept.id}
								className="inline-flex items-center gap-1 px-3 py-1.5 bg-teal-50 text-teal-700 rounded-full border border-teal-200"
								>
								{concept.display_name}
								<button
									type="button"
				...
				<div className="flex items-center gap-4 mb-6">
					...
					<span className="text-gray-400">y/o</span>
					<div className="flex-1 h-px bg-gray-200"></div>
				</div>
				)}
				...
				{searchMode === 'both' && (
					...
					<button
						type="button"    
						disabled={authorVal === ""}
						className="bg-indigo-400 hover:bg-indigo-600 px-4 py-1 text-white rounded-lg cursor-pointer disabled:bg-gray-300"
					...
					className="w-full py-3 px-4 bg-teal-600 hover:bg-teal-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-lg transition-colors cursor-pointer"
					>
